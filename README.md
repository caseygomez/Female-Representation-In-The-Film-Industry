...
### Credits
The credits dataset started with **45,476** rows and contained a column of data that contained the details of each cast member and another column with the details of each crew member for each movie id. First, the cast column was dropped and the crew column was exploded, so that the list of dictionaries became one dictionary per row. From there, the key/value pairs in each dictionary were turned into individual columns, and unnecessary columns were dropped. The crew dataset contained the gender information, and initial inspection showed that crew members were assigned a 0, 1, or 2 to denote gender. It was determined that 1 indicated female and 2 indicated male, but the dataset did not explain what the assignment of 0 indicated. In order to perform the planned calculations,   matched with females, 2 matched with males, but 0 had inconsistencies that needed to be corrected. 
